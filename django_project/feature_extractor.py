...
class FeatureExtractor:
    ...
    def extract(self, img):
        ...
        x = preprocessing.image.img_to_array(img)  # To np.array. Height x Width x Channel. dtype=float32
        x = np.expand_dims(x, axis=0)  # (H, W, C)->(1, H, W, C), where the first elem is the number of img
        x = applications.vgg16.preprocess_input(x)  # Subtracting avg values for each pixel
